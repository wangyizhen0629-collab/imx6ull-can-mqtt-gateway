...
{
  {0x100U,   10U, 0U, 0U, 0U, 0x10U, 0U},
  {0x101U,  100U, 0U, 0U, 0U, 0x20U, 0U},
  {0x102U, 1000U, 0U, 0U, 0U, 0x30U, 0U}
};
...
static void ECU_ProcessMessage(ECU_MessageStateTypeDef *message, uint32_t now);
                             ...
                             uint8_t payload[8])
{
  ...
  {
    payload[byte_index] = (uint8_t)(message->base +
                                    message->counter +
                                    byte_index);
    xor_value ^= payload[byte_index];
  }

  payload[6] = message->counter;
  xor_value ^= payload[6];
  payload[7] = xor_value;
}
...
int main(void)
{
  ...
  while (1)
  {
    ...
    ECU_ProcessMessage(&ecu_messages[0], now);
    ECU_ProcessMessage(&ecu_messages[1], now);
    ECU_ProcessMessage(&ecu_messages[2], now);
  }
  /* USER CODE END 3 */
}
