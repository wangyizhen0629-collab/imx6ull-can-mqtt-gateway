...
AUDIT_DIR="artifacts/20260829T134148+0800-m2-final-audit"
RUN_ID="20260829T133148+0800-m2-board-loopback"
RUN_DIR="artifacts/$RUN_ID"
ARCHIVE="artifacts/$RUN_ID.tar"
REEXTRACT_DIR="$AUDIT_DIR/archive_reextract"
MEMBERS_FILE="$AUDIT_DIR/archive_members.txt"
...
if [ -e "$REEXTRACT_DIR" ]; then
    echo "refusing to reuse archive re-extraction directory" >&2
    exit 2
fi
mkdir "$REEXTRACT_DIR"
tar -tf "$ARCHIVE" > "$MEMBERS_FILE"
tar --no-same-owner -xf "$ARCHIVE" -C "$REEXTRACT_DIR"
MEMBER_COUNT=0
while IFS= read -r MEMBER; do
    ...
    [ -f "$RUN_DIR/$RELATIVE" ]
    [ ! -L "$RUN_DIR/$RELATIVE" ]
    [ -f "$REEXTRACT_DIR/$MEMBER" ]
    cmp "$RUN_DIR/$RELATIVE" "$REEXTRACT_DIR/$MEMBER"
    MEMBER_COUNT=$((MEMBER_COUNT + 1))
done < "$MEMBERS_FILE"
[ "$MEMBER_COUNT" -eq 18 ]
pass "18 archive members match extracted raw file bytes; UID/GID metadata intentionally ignored"
