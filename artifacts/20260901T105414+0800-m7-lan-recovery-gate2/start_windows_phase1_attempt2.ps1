...
$attempt1Private = Join-Path $runRoot 'private_raw/windows-phase1/mosquitto.conf'
...
if (-not (Test-Path -LiteralPath $attempt1Private -PathType Leaf)) { throw 'Attempt-1 private Broker config is unavailable' }
...
if (Test-Path -LiteralPath $subscriberOutput) { throw 'Refusing to overwrite subscriber-phase1.jsonl' }
if (Get-NetTCPConnection -LocalPort $port -State Listen -ErrorAction SilentlyContinue) { throw "Port $port already has a listener" }

$attempt1Text = Get-Content -LiteralPath $attempt1Private
$listenerLine = $attempt1Text | Where-Object { $_ -match '^listener\s+18884\s+(.+)$' } | Select-Object -First 1
...
$configTest = Start-Process -FilePath $mosquitto -ArgumentList @('-c', $privateConfig, '--test-config') -NoNewWindow -Wait -PassThru -RedirectStandardOutput $configTestOut -RedirectStandardError $configTestErr
