...
$phase1Private = Join-Path $runRoot 'private_raw/windows-phase1-attempt2/mosquitto.conf'
$mosquitto = 'E:\mosquitto\mosquitto.exe'
$mosquittoSub = 'E:\mosquitto\mosquitto_sub.exe'
$port = 18884
$topic = 'test/m7/20260901T105414/recovery'
$subscriberOutput = Join-Path $runRoot 'subscriber-phase2.jsonl'

New-Item -ItemType Directory -Path $phaseRoot, $privateRoot -ErrorAction Stop | Out-Null
if (-not (Test-Path -LiteralPath $phase1Private -PathType Leaf)) { throw 'Phase1 private Broker config is unavailable' }
...
if (Test-Path -LiteralPath $subscriberOutput) { throw 'Refusing to overwrite subscriber-phase2.jsonl' }
if (Get-NetTCPConnection -LocalPort $port -State Listen -ErrorAction SilentlyContinue) { throw "Port $port already has a listener" }

$phase1Text = Get-Content -LiteralPath $phase1Private
$listenerLine = $phase1Text | Where-Object { $_ -match '^listener\s+18884\s+(.+)$' } | Select-Object -First 1
...
if (-not $listener) { throw 'Dedicated phase2 Broker listener did not become ready' }
...
@"
@echo off
"$mosquittoSub" -h $brokerHost -p $port -q 1 -t "$topic" -i "m7-win-sub-phase2-20260901T105414" -F "%%p" 1>"$subscriberOutput" 2>"$subscriberStderr"
...
if (-not $subscriberProcess) { throw 'Dedicated phase2 subscriber did not start' }
...
@(
    ...
    "subscriber_command=mosquitto_sub.exe -h <REDACTED_LAN_ADDRESS> -p $port -q 1 -t $topic -i m7-win-sub-phase2-20260901T105414 -F %p"
...
Write-Output "M7_WINDOWS_PHASE2_START_PASS broker_pid=$brokerPid subscriber_pid=$subscriberPid"
