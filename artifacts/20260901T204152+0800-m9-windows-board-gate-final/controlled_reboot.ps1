...
$RawPath = Join-Path $PrivateDir 'controlled_reboot.txt'
$PublicPath = Join-Path $RunDir 'controlled_reboot.redacted.txt'
$TracePath = Join-Path $RunDir 'controlled_reboot.redaction-trace.txt'
...
$BeforeOutput = & ssh.exe -o BatchMode=yes -o ConnectTimeout=10 "$BoardUser@$BoardHost" 'cat /proc/sys/kernel/random/boot_id; cat /proc/uptime; date -Ins' 2>&1 | Out-String
$BeforeExit = $LASTEXITCODE
$BeforeBootId = (($BeforeOutput -split "`r?`n")[0]).Trim()
$Lines.Add('command=ssh <REDACTED_USER>@<PRIVATE> cat boot_id,uptime,date before reboot')
$Lines.Add("exit_code=$BeforeExit")
$Lines.Add($BeforeOutput.TrimEnd())
if ($BeforeExit -ne 0 -or -not $BeforeBootId) { throw 'failed to obtain pre-reboot boot id' }

$RebootStarted = Get-Date -Format o
$RebootOutput = & ssh.exe -o BatchMode=yes -o ConnectTimeout=10 "$BoardUser@$BoardHost" 'sync; /sbin/reboot' 2>&1 | Out-String
$RebootExit = $LASTEXITCODE
$Lines.Add("reboot_command_started_at=$RebootStarted")
$Lines.Add('command=ssh <REDACTED_USER>@<PRIVATE> sync; /sbin/reboot')
$Lines.Add("exit_code=$RebootExit")
$Lines.Add($RebootOutput.TrimEnd())
