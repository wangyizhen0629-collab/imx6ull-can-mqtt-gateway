...
$Outputs = @(
    'powershell_syntax_check.v2.txt',
    'shell_syntax_check.v2.txt',
    'json_syntax_check.v2.txt',
    'git_diff_check.v2.txt',
    'repository_state.v2.txt',
    'raw_private_sha256.v2.txt',
    'docs_consistency.v2.txt',
    'sensitive_scan.v2.txt',
    'sensitive_scan.v2.exit.txt',
    'artifact_manifest.v2.sha256',
    'artifact_manifest_check.v2.txt'
)
...
[IO.File]::WriteAllLines((Join-Path $RunDir 'powershell_syntax_check.v2.txt'), $PsLines, [Text.UTF8Encoding]::new($false))
...
[IO.File]::WriteAllLines((Join-Path $RunDir 'shell_syntax_check.v2.txt'), $ShellLines, [Text.UTF8Encoding]::new($false))
...
[IO.File]::WriteAllLines((Join-Path $RunDir 'json_syntax_check.v2.txt'), $JsonLines, [Text.UTF8Encoding]::new($false))
...
[IO.File]::WriteAllText((Join-Path $RunDir 'git_diff_check.v2.txt'), $DiffText + "`r`n", [Text.UTF8Encoding]::new($false))
...
[IO.File]::WriteAllText((Join-Path $RunDir 'repository_state.v2.txt'), $RepoText + "`r`n", [Text.UTF8Encoding]::new($false))
...
[IO.File]::WriteAllLines((Join-Path $RunDir 'raw_private_sha256.v2.txt'), $PrivateHashLines, [Text.UTF8Encoding]::new($false))

$DocRequirements = [ordered]@{
    'README.md' = @('M9', 'NOT MET', 'M10尚未开始')
    'docs/DECISION_LOG.md' = @('20260901T204152+0800-m9-windows-board-gate-final', 'NOT MET', 'M10不得开始')
    'docs/OPEN_QUESTIONS.md' = @('M9', 'NOT MET', 'post-boot')
    'docs/PLANS.md' = @('20260901T204152+0800-m9-windows-board-gate-final', 'NOT MET', 'M10及后续未开始')
    'docs/PROJECT_SPEC.md' = @('M9', 'NOT MET', 'M10尚未开始')
    'docs/RESUME_TRACEABILITY.md' = @('20260901T204152+0800-m9-windows-board-gate-final', 'NOT MET', 'M10没有开始')
    'docs/TEST_PLAN.md' = @('20260901T204152+0800-m9-windows-board-gate-final', 'NOT MET', '不得开始M10')
    'docs/milestones/M9.md' = @('20260901T204152+0800-m9-windows-board-gate-final', 'NOT MET', 'M10没有开始')
}
$DocsLines = New-Object Collections.Generic.List[string]
$DocsFailures = 0
foreach ($Relative in $DocRequirements.Keys) {
    $Text = Get-Content -LiteralPath (Join-Path $RepoRoot $Relative) -Raw
    $Missing = @($DocRequirements[$Relative] | Where-Object { $Text.IndexOf($_, [StringComparison]::OrdinalIgnoreCase) -lt 0 })
    $Status = if ($Missing.Count -eq 0) { 'PASS' } else { 'FAIL' }
    if ($Status -eq 'FAIL') { $DocsFailures++ }
    $DocsLines.Add("file=$Relative required_count=$($DocRequirements[$Relative].Count) missing_count=$($Missing.Count) status=$Status")
    foreach ($Item in $Missing) { $DocsLines.Add("missing=$Item") }
}
$DocsLines.Add("total_failures=$DocsFailures")
[IO.File]::WriteAllLines((Join-Path $RunDir 'docs_consistency.v2.txt'), $DocsLines, [Text.UTF8Encoding]::new($false))
...
foreach ($Relative in $DocRequirements.Keys) { $ScanFiles.Add((Get-Item -LiteralPath (Join-Path $RepoRoot $Relative))) }
$EndpointLeak = 0
...
[IO.File]::WriteAllText((Join-Path $RunDir 'sensitive_scan.v2.txt'), $SensitiveText + "`r`n", [Text.UTF8Encoding]::new($false))
[IO.File]::WriteAllText((Join-Path $RunDir 'sensitive_scan.v2.exit.txt'), "$([int]($SensitiveTotal -ne 0))`r`n", [Text.Encoding]::ASCII)

$ManifestPath = Join-Path $RunDir 'artifact_manifest.v2.sha256'
$ManifestFiles = @(Get-ChildItem -LiteralPath $RunDir -File | Where-Object {
    $_.Name -notin @('artifact_manifest.v2.sha256', 'artifact_manifest_check.v2.txt')
} | Sort-Object Name)
...
[IO.File]::WriteAllLines((Join-Path $RunDir 'artifact_manifest_check.v2.txt'), $CheckLines, [Text.UTF8Encoding]::new($false))
