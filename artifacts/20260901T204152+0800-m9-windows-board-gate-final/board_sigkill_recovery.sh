...
count_verified()
{
    ...
    for p in /proc/[0-9]*; do
        ...
        if [ "$exe" = /opt/gatewayd/bin/gatewayd ]; then is_child=1; child_count=$((child_count + 1)); fi
        if [ "$comm" = gatewayd ] && [ "$is_supervisor" -eq 0 ] && [ "$is_child" -eq 0 ]; then unrelated_gatewayd_count=$((unrelated_gatewayd_count + 1)); fi
    done
}

echo "sigkill_started_at=$(date -Ins 2>/dev/null)"
verify_pair before_sigkill || { echo 'FAIL pre-signal identity'; exit 1; }
old_supervisor=$supervisor_pid
old_child=$child_pid
echo '--- child signal disposition before SIGKILL ---'
grep -E '^(Name|Pid|PPid|State|SigPnd|ShdPnd|SigBlk|SigIgn|SigCgt):' "/proc/$old_child/status" 2>&1
stat -c 'spool_mode=%a uid=%u gid=%g size=%s mtime=%Y path=%n' /var/lib/gatewayd/20260901T204152p0800-m9-board/spool.data 2>&1

kill -KILL "$old_child" 2>&1
kill_rc=$?
echo "command=kill -KILL $old_child exit=$kill_rc"
[ "$kill_rc" -eq 0 ] || exit 1

attempts=20
while [ "$attempts" -gt 0 ]; do
    new_child=$(sed -n '1p' /var/run/gatewayd/gatewayd.pid 2>/dev/null)
    if [ -n "$new_child" ] && [ "$new_child" != "$old_child" ] && [ -d "/proc/$new_child" ]; then break; fi
    attempts=$((attempts - 1))
    sleep 1
done
[ "$attempts" -gt 0 ] || { echo 'FAIL replacement child timeout'; exit 1; }
verify_pair after_sigkill || gate=1
...
count_verified
echo "after_sigkill_supervisor_count=$supervisor_count"
echo "after_sigkill_verified_child_count=$child_count"
echo "after_sigkill_unrelated_gatewayd_count=$unrelated_gatewayd_count"
[ "$supervisor_count" -eq 1 ] || gate=1
[ "$child_count" -eq 1 ] || gate=1
[ "$unrelated_gatewayd_count" -eq 0 ] || gate=1

sleep 5
verify_pair after_sigkill_stable || gate=1
...
echo "sigkill_recovery_gate=$(if [ "$gate" -eq 0 ]; then echo PASS; else echo FAIL; fi)"
echo "sigkill_completed_at=$(date -Ins 2>/dev/null)"
exit "$gate"
