...
# 验证本run中的PowerShell脚本可解析；本run没有shell脚本。
...
Get-ChildItem -LiteralPath $RunDir -Filter '*.ps1' -File | Sort-Object Name | ForEach-Object {
    ...
    if ($Errors.Count -eq 0) {
        ...
        $PsLines += "$($_.Name)=FAIL error_count=$($Errors.Count)"
        $PsErrors += $Errors.Count
    }
}
$PsLines += "result=$(if ($PsErrors -eq 0) { 'PASS' } else { 'FAIL' })"
Write-Lines (Join-Path $RunDir 'powershell_syntax_check.txt') $PsLines
...
Write-Lines (Join-Path $RunDir 'json_syntax_check.txt') $JsonLines
...
foreach ($Marker in $Markers) {
    ...
    $MarkerLines += "marker_sha256=$(([Security.Cryptography.SHA256]::Create().ComputeHash($Utf8.GetBytes($Marker)) | ForEach-Object { $_.ToString('x2') }) -join '') result=$(if ($Present) { 'PASS' } else { 'FAIL' })"
...
Write-Lines (Join-Path $RunDir 'evidence_marker_check.txt') $MarkerLines
...
foreach ($Entry in $DocRequirements.GetEnumerator()) {
    $DocPath = Join-Path $RepoRoot $Entry.Key
    ...
    if ($Missing.Count -ne 0) { $DocFailures += $Missing.Count }
}
...
Write-Lines (Join-Path $RunDir 'docs_consistency.txt') $DocLines
if ($DocFailures -ne 0) { $Failed = $true }

$DiffOutput = @(& git -C $RepoRoot diff --check 2>&1)
$DiffExit = $LASTEXITCODE
$DiffLines = @("exit_code=$DiffExit") + $DiffOutput
$DiffLines += "result=$(if ($DiffExit -eq 0) { 'PASS' } else { 'FAIL' })"
Write-Lines (Join-Path $RunDir 'git_diff_check.txt') $DiffLines
...
$StateLines = @(
    ...
    "tracked_status_line_count=$($TrackedState.Count - 1)"
    "untracked_count=$UntrackedCount"
    "tmp_evidence_status=$($TmpStatus -join ' ')"
    "artifact_status=$($ArtifactStatus -join ' ')"
    'tracked_status:'
) + $TrackedState
Write-Lines (Join-Path $RunDir 'repository_state.txt') $StateLines
...
Write-Lines (Join-Path $RunDir 'raw_private_sha256.txt') $RawHashLines
...
$SensitiveLines = @(
    ...
    "result=$(if ($LeakCount -eq 0 -and $Login.Success) { 'PASS' } else { 'FAIL' })"
)
Write-Lines (Join-Path $RunDir 'sensitive_scan.txt') $SensitiveLines
$SensitiveExit = if ($LeakCount -eq 0 -and $Login.Success) { 0 } else { 1 }
Write-Lines (Join-Path $RunDir 'sensitive_scan.exit.txt') @("exit_code=$SensitiveExit")
...
Write-Lines $ManifestCheckPath $ManifestCheckLines
