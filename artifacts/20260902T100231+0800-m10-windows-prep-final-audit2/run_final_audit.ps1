...
Get-ChildItem -LiteralPath (Join-Path $mdkRoot 'M10_500') -Filter '*.d' -File | ForEach-Object {
    foreach ($line in Get-Content -LiteralPath $_.FullName) {
        if ($line -match '^[^:]+:\s+(.+)$') {
            ...
            if ($dependency -notmatch '^[A-Za-z]:\\') { $dependencies += $dependency }
        }
    }
}
$uniqueDependencies = @($dependencies | Sort-Object -Unique)
foreach ($dependency in $uniqueDependencies) {
    $absolute = [IO.Path]::GetFullPath((Join-Path $mdkRoot $dependency))
    $relative = $absolute.Substring($repoRoot.Length + 1).Replace('\', '/')
    $trackedMatches = @(git ls-files -- $relative)
    $tracked = ($trackedMatches.Count -eq 1)
    $intendedNew = ($relative -eq $newHeader)
    $dependencyLines += "tracked=$tracked intended_new=$intendedNew path=$relative"
    if (-not $tracked -and -not $intendedNew) { $dependencyFailures += $relative }
}
$dependencyLines += "dependency_count=$($dependencies.Count) unique_count=$($uniqueDependencies.Count) failure_count=$($dependencyFailures.Count)"
...
$previousManifest = @()
$previousManifest += Test-Manifest -Directory (Join-Path $repoRoot 'artifacts/20260902T094551+0800-m10-windows-profile-prep')
$previousManifest += Test-Manifest -Directory (Join-Path $repoRoot 'artifacts/20260902T094824+0800-m10-windows-profile-prep2')
$previousManifest += Test-Manifest -Directory (Join-Path $repoRoot 'artifacts/20260902T095951+0800-m10-windows-prep-final-audit')
...
foreach ($target in @('M10_111', 'M10_500', 'M10_1000')) {
    ...
    if (-not $ignored -or $trackedProducts.Count -ne 0) { throw "Build product scope check failed for $target" }
}
Write-NewUtf8 -Name 'build_product_git_audit.txt' -Value $ignoreLines

$oldPreference = $ErrorActionPreference
$ErrorActionPreference = 'Continue'
$diffOutput = git diff --check 2>&1 | ForEach-Object { $_.ToString() }
...
$intended = @(
    ...
    'artifacts/20260902T100231+0800-m10-windows-prep-final-audit2/'
)
Write-NewUtf8 -Name 'intended_commit_scope.txt' -Value $intended

$scanFiles = @()
foreach ($entry in $intended) {
    if (Test-Path -LiteralPath $entry -PathType Leaf) { $scanFiles += Get-Item -LiteralPath $entry }
    elseif (Test-Path -LiteralPath $entry -PathType Container) { $scanFiles += Get-ChildItem -LiteralPath $entry -File -Recurse }
}
...
$keyFiles = @(
    ...
    'stm32/firmware/imx6ull-can-mqtt-gateway/Core/Inc/ecu_traffic_profile.h',
    'stm32/firmware/imx6ull-can-mqtt-gateway/Core/Src/main.c', $projectRel,
